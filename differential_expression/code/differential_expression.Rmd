...
Predict gene names for each transcript in the de novo transcriptomes using EggNOG Mapper.
...
Generate list of transcripts that are annotated, have e-values less than 1e-30, and share gene names across all lineages. 
...
june_all <- read_tsv(june_path) %>%
  ...
  select(queryID, geneName, eValue, bitScore) %>% mutate(geneName = toupper(geneName))
...
common_genes1 <- intersect(intersect(june_gene_name, utah_gene_name), straw_gene_name)

common_genes_up[duplicated(common_genes_up)]
