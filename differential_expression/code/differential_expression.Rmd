...
candidate_expression_combined <- arrange(candidate_expression_combined, gene)
...
genePvalue <- deseqRes2Tibble(straw_res_14v12) %>% filter(gene == "nipbl_1")

```

# Candidate Transcript Plots

## All Candidate Transcripts
...
gene_counts <- function(j_dds, u_dds, s_dds, gene_name, significance) {
  ...
  ggplot(counts, aes(x = Time, y = count, color = species)) + 
    ...
    theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), 
          plot.title = element_text(hjust = 0.5, size = 6, face = "italic"), axis.title.x = element_blank(),
    ...
    add_pvalue(j_significance, label = "signif", xmin = "Time2", xmax = "Time1", 
               ...
               label.size = 2, bracket.color = "black", bracket.size = .2) +
    add_pvalue(u_significance, label = "signif", xmin = "Time2", xmax = "Time1", 
               ...
               label.size = 2, bracket.color = "transparent") +
...
# Number of candidate genes vs. transcripts

paste("transcripts", length(plotCombo))
paste("genes", length(unique(gsub('(.+)_.+','\\1', plotCombo))))

# Plot all differentially expressed transcripts

multiGene_plot(plotCombo, "all_transcripts")

```

## Plots of select transcripts 

```{r}

threeGene_counts <- function(j_dds, u_dds, s_dds, gene_name, significance) {
  ...
  ggplot(counts, aes(x = Time, y = count, color = species)) + 
    ...
    ggtitle(gene_name) +
    theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), 
          plot.title = element_text(hjust = 0.5, size = 12, face = "italic"), axis.title.x = element_blank(),
          axis.title.y = element_blank(), axis.text.x = element_text(size = 7),
          axis.text.y = element_text(size = 7)) +
    add_pvalue(j_significance, label = "signif", xmin = "Time2", xmax = "Time1", 
               y.position = log10(1.5*max(counts$count)), tip.length = .02, step.increase = 0.06,
               label.size = 4, bracket.color = "black", bracket.size = .3) +
    add_pvalue(u_significance, label = "signif", xmin = "Time2", xmax = "Time1", 
               y.position = log10(1.55*max(counts$count)), tip.length = .02, step.increase = 0.06,
               label.size = 4, bracket.color = "transparent") +
    add_pvalue(s_significance, label = "signif", xmin = "Time2", xmax = "Time1", 
               y.position = log10(1.65*max(counts$count)), tip.length = .02, step.increase = 0.06,
               label.size = 4, bracket.color = "transparent") 
}
...
threeGene_plot <- function(interest, str1) {
  ...
  for (i in interest) {
    ...
    if (!any(is.na(pValue_Final$signif))) {
      gene_plot <- threeGene_counts(june_dds, utah_dds, straw_dds, i, pValue_Final)
      gene_list[[i]] <- gene_plot
    }
  }
  
    figlist <- ggarrange(plotlist = gene_list, ncol=3, common.legend = TRUE, legend="bottom") 
    figlist <- list(figlist)
    final_pdf <- marrangeGrob(figlist[1], nrow = 1, ncol = 1, left = "Counts", bottom = "Time", top = NULL)
    pdf_name <- paste("./figures/counts/", str1, ".pdf", sep ="")
    pdf_name <- gsub(" ", "", pdf_name)   
    ggsave(file = pdf_name, final_pdf, width = 8.5, height = 4, units = "in") 
  
}

threeGene_plot(c("mmp2_1", "mmp2_2", "nipbl_1"), "three_transcripts")
