...
nrow(june_txi$counts) == 7195
nrow(utah_txi$counts) == 7195
nrow(straw_txi$counts) == 7195
```
...
Generate a comprehensive list of candidate genes using Geneontology.org.
...
### Generate Vectors of Candidate Genes

```{r}

# Generate vector of all genes
...
search <- GO_genes %in% all_genes

#final_genes <- character()

#for (gene in GO_genes) {
#  pattern <- paste0("^", gene, "_")
#  final_genes <- append(grep(pattern, all_genes, value = TRUE), geneV)
#}

# Generate vector of all candidate genes 

final_genes <- tibble(GO_genes, search) %>%
  filter(search == TRUE) %>%
  pull(GO_genes) %>%
  as.vector()
```

### Generate Tables of Candidate Genes and Expression Levels

These 43 candidate genes were then analyzed between the contrasts of 12v9, 14v12 and 14v9 to determine if the gene was upregulated (1), downregulated (-1), unchanged (0) or undetermined (padj = NA). Genes were then removed if their padj equaled NA or if they were in the "0 0 0 0 0 0" category because this indicated the genes were unchanged across all contrasts. This left me with 27 final candidate genes.
...
# Remove any genes that have an NA for their padj from all time combinations (12v9, 14v12, 14v9).

candidate_expression_combined <- drop_na(candidate_expression_combined)

# Generate combo column with expression combinations of each gene (i.e., 0 0 0 0 0 1 = Upregulated in straw in 14v12, unchanged for all other species and times). Currently, only includes 12v9 & 14v12. Code below can easily be modified to include 14v9 information and extend the combination. However, for now I'm just basing my combinations off of 12v9 and 14v12. This makes the interpretation much simpler!
...
# Generate counts of expression combinations to explore clustering of genes 
...
check <- filter(candidate_expression_combined, combo == "0 1 0 0 -1 0") %>% select(gene) %>% pull(gene)

# Look at p-value for specific gene. This can be useful to confirm that other functions are working appropriately. 

genePvalue <- deseqRes2Tibble(straw_res_14v12) %>% filter(gene == "SKI")
```

# Candidate Gene Plots

## Plot Counts (Gene-level)

Generated normalized counts using plotCounts and then generated my own plot using ggplot2 with the data frame plotCounts returned. 

The biggest issue with this plot (single and multiple gene plots) is that the scaling can be off. I will have to manually fix this if I use these plots. 
...
# Generate plot of gene counts for gene of interest
...
gene_counts <- function(j_dds, u_dds, s_dds, gene_name, significance) {
  ...
  ggplot(counts, aes(x = Time, y = count, color = species)) + 
    ...
    scale_y_log10(limits = c(NA, 6*max(counts$count))) +
    scale_colour_discrete("Species") +
    theme_bw() +
    theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), 
          plot.title = element_text(hjust = 0.5, size = 9), axis.title.x = element_blank(),
...
#### Plots of genes by Combo of Interest
...
# Generates multiple plots for combination of interest
...
multiGene_plot <- function(interest, str1) {
  ...
  # Arrange plots into groups of 8 per page and store in a list
  
  # If length of genelist is <= 8 & != 0, tempfiglist must be stored in figlist (i.e. nested list) for marrange to work
  
  if (length(gene_list) <= 8 & length(gene_list) != 0) {
    tempfiglist <- ggarrange(plotlist = gene_list, ncol=4, nrow=2, 
                             common.legend = TRUE, legend="bottom") 
    figlist <- list(tempfiglist)
    final_pdf <- marrangeGrob(figlist, nrow = 1, ncol = 1, top = str1, left = "Counts", bottom = "Time")
    pdf_name <- paste("./figures/counts/", str1, ".pdf", sep ="")
    pdf_name <- gsub(" ", "", pdf_name)   
    ggsave(file = pdf_name, final_pdf)
    
  } else if (length(gene_list) != 0) {
    figlist <- ggarrange(plotlist = gene_list, ncol=4, nrow=2, common.legend = TRUE, legend="bottom") 
    final_pdf <- marrangeGrob(figlist, nrow = 1, ncol = 1, top = str1, left = "Counts", bottom = "Time")
    pdf_name <- paste("./figures/counts/", str1, ".pdf", sep ="")
    pdf_name <- gsub(" ", "", pdf_name)   
    ggsave(file = pdf_name, final_pdf)
...
```{r}
# Determine expression combinations of interest from all candidate genes
  
combination <- dplyr::count(candidate_expression_counts, combo) %>% pull(combo)

#combination <- combination[combination != "0 0 0 0 0 0"]

# It can really speed up the time it takes to execute the code below if the combination where all genes are unchanged is removed. You can use the following code to do so: combination <- combination[combination != "0 0 0 0 0 0"]

# Loop through all specific expression combinations of interest and generate PDFs of plots

for (i in combination) {
  
  plotCombo <- filter(candidate_expression_combined, combo == i) %>% select(gene) %>% pull(gene)
  
  multiGene_plot(plotCombo, i)
  
  }
```

#### Plots of all 28 genes
...
# The functions below have been modified from functions above so that all 28 plots would fit onto 3 pages and that none of the brackets and asterisks were lost
...
multiGene_plot <- function(interest, str1) {
  ...
  if (length(gene_list) <= 8 & length(gene_list) != 0) {
    tempfiglist <- ggarrange(plotlist = gene_list, ncol=3, nrow=3, 
                             common.legend = TRUE, legend="bottom") 
    figlist <- list(tempfiglist)
    ...
    figlist <- ggarrange(plotlist = gene_list, ncol=3, nrow=3, common.legend = TRUE, legend="bottom") 
    final_pdf <- marrangeGrob(figlist, nrow = 1, ncol = 1, left = "Counts", bottom = "Time", top = NULL)
    pdf_name <- paste("./figures/counts/", str1, ".pdf", sep ="")
    pdf_name <- gsub(" ", "", pdf_name)   
    ggsave(file = pdf_name, final_pdf, width = 8.5, height = 11, units = "in") # figlist,
