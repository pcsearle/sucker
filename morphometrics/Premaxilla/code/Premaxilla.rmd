...
require("knitr")
opts_knit$set(root.dir = "/home/searleps/Sucker/Geometric_Morphometrics/Morphometrics/Premaxilla/")
...
setwd("/home/searleps/Sucker/Geometric_Morphometrics/Premaxilla/")
...
The following plot is of the length of the vertical and lateral limbs of the premaxilla. It shows that premaxilla size changes during development.

## Determine interlandmark distance 
...
vertical <- group_by(all, Species, Combo) %>% summarise(vMean = mean(vertical), vSd = sd(vertical))

lateral <- group_by(all, Species, Combo) %>% summarise(lMean = mean(lateral), lSd = sd(lateral))

comboWks <- full_join(vertical, lateral) %>% mutate(Spcs = factor(Species, c(1, 2, 3), c("June", "Utah", "Strawberry")))

# Ratios
# If the ratio of these is similar, that indicates that the shape (i.e., lengths of landmarks) is roughly the same

comboWks <- mutate(comboWks, ratio = vMean/lMean)

comboWks$Combo <- as.factor(comboWks$Combo)
...
# Vertical vs. Lateral Length

myPlot <- ggplot(comboWks, aes(lMean, vMean)) +
  geom_point(aes(color = Spcs, shape = Combo)) +
  geom_path(aes(color = Spcs)) +
  xlab("Lateral") +
  ylab("Vertical") +
  geom_errorbar(aes(ymin=vMean-vSd, ymax=vMean+vSd), width = .1, color = gray(0, .1)) +
  geom_errorbar(aes(xmin=lMean-lSd, xmax=lMean+lSd), width = .1, color = gray(0, .1)) +
